...
<div class="container">
    <!-- start header -->
    <header class="header">
        <h1 class="text-capitalize"><?php echo language('UPDATE COMBINATION DETAILS') ?></h1>
        <?php
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($checkComb > 0) {
                ...
                $msg  = '<div class="alert alert-success text-capitalize"><i class="bi bi-check-circle-fill"></i>&nbsp;'.language("THERE IS NO COMBINATIONS TO SHOW", $_SESSION['systemLang']).'</div>';
                redirectHome($msg);
            }
        } else {
            // log message
            $logMsg = "Combination dept: you cannot access this page directly!.";
            createLogs($_SESSION['UserName'], $logMsg);
            // error message
