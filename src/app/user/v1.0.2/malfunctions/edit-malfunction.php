...
if ($count > 0) {
    ...
    if ($malrows['isShowed'] == 0) {
        if ($_SESSION['UserID'] == $malrows['tech_id']) {
            ...
            $q = "UPDATE `malfunctions` SET `isShowed` = 1, `showed_date` = CURRENT_DATE, `showed_time` = CURRENT_TIME WHERE `mal_id` = ?";
            $stmt = $con->prepare($q);     
            $stmt->execute(array($malID));               // execute data 
        }
    }
?>
    ...
    <div class="container">
        <!-- start header -->
        <header class="header">
            <h1 class="text-capitalize"><?php echo language('SHOW MALFUNCTION DETAILS', $_SESSION['systemLang']) ?></h1>
            <!-- <span>Technical man <span class="badge bg-danger fs-12 fw-light rounded-pill"><?php echo $malrows['isShowed'] == 1 ? "showed" : "not showed" ?></span> this malfunction</span> -->
        </header>
        <!-- end header -->
        <!-- start form -->
        <form class="py-3 my-5 custom-form needs-validation" action="?do=updateMalfunction" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="section-block">
                        ...
                        <div class="mb-sm-2 mb-md-3 row">
                            ...
                            <div class="col-sm-12 col-md-8">
                                ...
                                <a href="malfunctions.php?do=showPiecesMal&pieceid=<?php echo $malrows['client_id'] ?>" class="mt-2 text-start" dir="<?php echo $_SESSION['systemLang'] == "ar" ? "rtl" : "ltr" ?>"><?php echo language("SHOW DETAILS", $_SESSION['systemLang']) ?></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- submit -->
            <div class="my-3 row row-cols-sm-2 row-cols-md-6 g-3">
                <div class="ms-auto col-6">
                    <button type="submit" class="btn btn-primary text-capitalize form-control bg-gradient" id="update-malfunctions" <?php if ($_SESSION['mal_update'] == 0 && $malrows['isReviewed'] == 1) {echo 'disabled';} ?>>
                        <i class="bi bi-check-all"></i>&nbsp;<?php echo language('SAVE CHANGES', $_SESSION['systemLang']) ?>
                    </button>
                </div>
                <div class="col-6">
                    <button type="button" class="btn btn-outline-danger text-capitalize form-control bg-gradient " data-bs-toggle="modal" data-bs-target="#deleteMalModal" <?php if ($_SESSION['mal_delete'] == 0) {echo 'readonly';} ?> >
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="?do=deleteMal&malid=<?php echo $malrows['mal_id'] ?>" class="btn btn-danger text-capitalize <?php if ($_SESSION['mal_delete'] == 0) {echo 'disabled';} ?>" ><i class="bi bi-trash"></i>&nbsp;<?php echo language('DELETE', $_SESSION['systemLang']) ?></a>
                    </div>
                </div>
            </div>
        </div>
...
<?php } else { ?>
    ...
    <div class="container">
        ...
        <header class="header">
            <h1 class="text-capitalize"><?php echo language('SHOW MALFUNCTION DETAILS', $_SESSION['systemLang']) ?></h1>
            <?php
                $msg = "<div class='alert alert-warning'>".language('THERE IS NO MALFUNCTIONS TO SHOW', $_SESSION['systemLang'])."</div>";
