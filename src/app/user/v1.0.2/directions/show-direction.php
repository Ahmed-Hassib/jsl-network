...
// query select
$q = "SELECT `pieces`.`piece_id`, `pieces`.`piece_ip`, `pieces`.`piece_name`, `pieces`.`source_id`, `pieces`.`type_id`, `direction`.`direction_name`, `pieces`.`direction_id` FROM `pieces` LEFT JOIN `direction` ON `direction`.`direction_id` = `pieces`.`direction_id` WHERE `direction`.`direction_id` = ?";


$stmt = $con->prepare($q);          // select all users
$stmt->execute(array($dirid));      // execute data
...
<div class="container" name="showDir">
    <!-- start header -->
    <header class="header">
        <h1 class="text-capitalize"><?php echo language('SHOW', $_SESSION['systemLang'])." ".language('THE DIRECTION', $_SESSION['systemLang']) ?></h1>
        <h3 class="h3 text-secondary "><?php echo $dirname ?></h3>
    </header>
    <!-- end header -->
    <!-- start showing directions pieces -->
</div>
<div class="mt-5">
    <div class="h-100 tree-container" style="overflow-x: scroll">
        <div class="tree">
            <?php
                if (count($data) > 0) {
