    ...
    <div class="modal-dialog modal-dialog-centered modal-type-dialog" dir="<?php echo $_SESSION['systemLang'] == 'ar' ? 'rtl' : 'ltr' ?>">
        <div class="modal-content">
                ...
                <form action="?do=piecesTypes&action=updatePieceType" method="POST" id="editPieceType">
                    ...
                    <div class="mb-sm-2 mb-md-3 row">
                        ...
                        <div class="col-sm-12 col-md-8">
                            <?php $typesRows = selectSpecificColumn("*", "`types`", ""); ?>
                            <select class="form-select" id="old-type-name" name="old-type-name" onchange="document.getElementById('type-id').value = this.value; document.getElementById('new-type-note').value = this[this.selectedIndex].dataset.note;" required>
                                ...
                                <?php foreach ($typesRows as $type) { ?>
                                    <option value="<?php echo $type['type_id'] ?>" data-note="<?php echo $type['type_note'] ?>"><?php echo $type['type_name'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary py-1 fs-12" form="editPieceType" onclick="validateForm(this.form)"><?php echo language("SAVE CHANGES", $_SESSION['systemLang']) ?></button>
