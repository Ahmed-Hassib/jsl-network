...
function getTitle() {
    ...
    // check if set or not
    if (isset($pageTitle)) {
        echo (isset($_SESSION['company_name']) ? $_SESSION['company_name'] . ' | ' : '') . strtoupper(language(strtoupper($pageTitle), isset($_SESSION['systemLang']) ? $_SESSION['systemLang'] : "ar"));
    } else {
 ...
 */
function buildTreeview($arr, $parent, $level = 0, $prelevel = -1, $nav_up_level = 1) {
    foreach ($arr as $id => $data) {
        if ($parent == $data['source_id']) {
            // check if this record is main
            $data['src'] = $parent == 0 ? $data['piece_ip'] : $arr[$parent]['piece_ip'];
            // check tree level
            if ($level > $prelevel) {
                echo "<ul>";
            }
            ...
            $node_url = str_repeat("../", $nav_up_level) . "pieces/index.php?name=pieces&do=showPiece&dirId=" . $data['direction_id'] . "&srcId=" . $data['piece_id'];
            
            // show data
            echo "<li>";
            echo "<a href='$node_url'>";
            echo "<span id=". $data['piece_ip'] .">" . $data['piece_name'] . "<br>" . $data['piece_ip'] . "</span>";
            echo "</a>";
